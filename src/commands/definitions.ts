...
5. **Smart Delegation loop** — announce the delegation plan to the user, then drive the loop, narrating each step:
   ...
   c. \`cerebro_verify\` each task — the only path to \`verified\`. On FAIL it auto-requeues the task (→ \`pending\`, \`attempts\` tracked) or auto-blocks it at the retry budget; re-dispatch requeued tasks through the loop with the recorded failure output — don't track retries or mark blocked yourself.
