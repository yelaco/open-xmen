...
5. **Legion** (product-shaped or user-facing work only): run Legion via the native \`task\` tool (subagent_type = the agent) to produce customer vision. Pass Legion's \`CUSTOMER_VISION_READY\` to Cypher as context.
...
8. **Beast**: gap review via the native \`task\` tool (subagent_type = the agent). **Emma Frost**: validate via the native \`task\` tool (subagent_type = the agent) if HIGH risk, public API, auth, data, billing, or migration work.
...
5. **Smart Delegation loop** — announce the delegation plan to the user, then drive the loop, narrating each step:
   ...
   b. Spawn each ready task with the native \`task\` tool (\`subagent_type\` = the returned \`agent\`, \`prompt\` = the task context); launch independent ready tasks concurrently (multiple \`task\` calls in one message) to run them in parallel. Each subagent runs in its own visible session and returns when done — no polling. Run chains in order (jean-grey → wolverine → storm), threading the spec/component paths forward.
   ...
   d. Narrate the step to the user (what ran, what verified, what's next), then repeat from (a) until \`cerebro_next_tasks\` returns nothing ready.
6. **Independent Verification** — when nothing is ready and 0 remain, spawn **Cyclops** via the \`task\` tool (\`subagent_type: cyclops\`) for the final audit, giving it the objective, task + verification summary, and acceptance criteria; read its \`AUDIT_PASSED\`/\`AUDIT_FAILED\` verdict. On AUDIT_FAILED, record findings with \`cerebro_problem_report\`, re-queue retriable ones (\`cerebro_task_update\` → pending) and resume the loop; escalate non-retriable findings. Never skip the audit.
...
9. **Legion acceptance** (product-shaped plans only): if the plan includes user-facing acceptance criteria or was derived from Legion/Cypher notepads, run Legion via the native \`task\` tool (subagent_type = the agent) for a final customer acceptance verdict. A \`CUSTOMER_VERDICT: REJECT\` creates retry tasks via \`cerebro_task_create\` and re-enters the loop.
...
5. **Legion** (product-shaped work only): run Legion via the native \`task\` tool (subagent_type = the agent) to produce customer vision from the prompt and codebase. Legion writes \`CUSTOMER_VISION_READY\` under \`.cerebro/notepads/customer/\` — no user questions.
6. **Cypher** (\`MODE: autonomous\`): run Cypher via the native \`task\` tool (subagent_type = the agent) with the original prompt, intent sub-type, Legion's vision (if produced), and \`MODE: autonomous\`. Cypher produces \`REQUIREMENTS_READY\` directly — using safe defaults and documenting all assumptions. No CLARIFY rounds.
...
10. **Smart Delegation loop** — announce the delegation plan, then drive the loop, narrating each step: (a) \`cerebro_next_tasks\` for the ready batch with routing; (b) spawn each via the native \`task\` tool (\`subagent_type\` = the routed \`agent\`), launching independent tasks concurrently for parallelism, running visual-engineering chains in order; (c) \`cerebro_verify\` each (the only path to \`verified\`; on FAIL re-spawn with the failure output, ≤2 retries, else \`cerebro_task_update\` → blocked); (d) narrate the step and repeat until nothing is ready.
11. **Independent Verification** — spawn **Cyclops** via the \`task\` tool (\`subagent_type: cyclops\`) for the final audit; read its \`AUDIT_PASSED\`/\`AUDIT_FAILED\` verdict. On AUDIT_FAILED, record findings with \`cerebro_problem_report\`, re-queue retriable ones (\`cerebro_task_update\` → pending) and resume the loop; escalate non-retriable findings autonomously where possible. Never skip the audit.
12. **Legion acceptance** (product-shaped work only): run Legion via the native \`task\` tool (subagent_type = the agent) for a final customer verdict. A \`CUSTOMER_VERDICT: REJECT\` creates retry tasks and re-enters the loop.
