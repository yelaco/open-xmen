...
9. Create task records with \`cerebro_task_create\` — each with \`category\`, \`depends_on\`, \`files\`, and \`verification_commands\` from the plan (and \`effort\` low/high when a task is trivial or hard) — then **run the workflow engine**: call \`cerebro_execute_workflow\` with the run_id and \`plan_path\`. The deterministic engine schedules dependency frontiers, routes by category, fans out independent tasks in parallel, runs every task's \`Verify\` commands in a shell, retries failures (max 2 per task), and finishes with a Cyclops audit wave that rules AUDIT_PASSED or AUDIT_FAILED. **Never dispatch workers or Cyclops yourself; never bypass the engine.**
