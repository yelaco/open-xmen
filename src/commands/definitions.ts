...
6. **Cypher** (\`MODE: autonomous\`) → \`REQUIREMENTS_READY\` (defaults that serve Legion's quality bar — ambitious on quality, safe on risk; documented assumptions; no CLARIFY rounds), seeded with the prompt, intent sub-type, and Legion's vision if produced ⇐ run only when requirements are ambiguous or acceptance criteria need pinning.
