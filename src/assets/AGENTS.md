...
- Cerebro orchestrates the execution loop itself (`cerebro_next_tasks` → spawn specialists via the native `task` tool → `cerebro_verify` → final Cyclops audit, also a `task` spawn), narrating each step; determinism lives in those tools.
